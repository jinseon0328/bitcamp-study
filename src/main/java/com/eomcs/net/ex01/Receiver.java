...
public class Receiver {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // - 포트번호 : 1024 ~ 49151 사이의 값 사용한다.
    // - 같은 컴퓨터에서 이미 사용중인 포트 번호는 지정할 수 없다.
    // - 예) Oracle DBMS(1521), MY SQL(3306) 등
    ServerSocket serverSocket = new ServerSocket(8888);
    //                                          ------ 포트 번호
    System.out.println("서버 소켓 생성!");

    // 2) 연결하기 위해 대기하고 있는 클라이언트 중에서 중에서 한 개를 연결 허락한다.
    ...
    //                                      ----------------outputStream()를 구현한 class의 객체를 리턴한다
    Scanner in = new Scanner(socket.getInputStream());
    // Scanner는 입력 스트림이 아니기 때문에 데코레이터 같아 보이지만 데코레이터는 아니다
