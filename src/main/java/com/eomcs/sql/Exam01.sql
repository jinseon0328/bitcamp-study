...
- 트리거(trigger)
  ...
  - 특정 조건? SQL 실행 전/후 등
  - OOP 디자인 패턴에서 옵저버에 해당한다. 리스너와 같다
- 함수(function)
- 프로시저(procedure)
- 인덱스(index)
...
use studydb;
이렇게 안 쓰고 바로 쓸 때는
create studydb.test1 table ();
이런식으로 앞에 데이터베이스. 을 입력해주어야 한다
...
> create table 테이블명 (
  ...
  컬럼명 타입 NULL여부 옵션
        ...
        데이터타입 -> 오라클과 조금씩 다르다. ex) date = 날짜, 시간 / date = 날짜
        옵션 값을 안 넣는다면 뭘 집어넣을지
...
> create table test01 (
    ...
    varchar = variable char 변동하는 char (영어도 50자 한글도 50자 = 최대 글자수)
                 integer과 int 큰 정수는 bigint 써도 상관없다
...
- 메모리 크기가 고정되어서 데이터를 찾을 때 위치파악이 빠르다.
- oracle은 char(5) 라고 저장하면 1글자만 넣어도 'a    '으로 들어간다. 그래서 비교해도 'a    ' 전체가 맞아야 한다.

#### varchar(n)
- 최대 n개의 문자를 저장.
- 0 ~ 65535 바이트 크기를 갖는다.
- n 값은 문자집합에 따라 최대 값이 다르다.
- 한 문자에 1바이트를 사용하는 ISO-8859-n 문자집한인 경우 최대 65535 이다. 65535 / 3 = 21844
- 그러나 UTF-8로 지정된 경우는, n은 최대 21844까지 지정할 수 있다. 보통 3바이트를 쓰므로
- 가변 크기를 갖는다.
...
> insert into test1(c3) values(4); 
/* 4 -> 4 다른 숫자도 들어가기는 하지만 나중에 리턴 받을 때 제대로 출력이 안될 수 있다*/

- 숫자 컬럼인 경우 값을 설정할 때 문자로 표현할 수 있다.
- 즉 문자열을 숫자로 바꿀 수 있으면 된다.

### 키 컬럼 지정
/*어떤 값들을 비교할 때 구별하는 식별자 = 키*/
/*데이터에 결점이 없는 것 = 무결성 (data integrity) */
...
- 왜?
  ...
  - 이메일 변경,
    - primary key 값은 다른 데이터에서 참조하는 용도로 사용하기 때문에,
      - 예) 게시글을 저장할 때 회원 이메일을 저장한다고 가정하자.
            이메일이 바뀌면 모든 게시글의 이메일을 바꿔야 하므로
...
/* 또는 실무에서는 다음과 같이 테이블 정의 후 제약 조건을 모든 컬럼 선언 뒤에 놓을 수 있다. */
> create table test1(
  no int,
  name varchar(20),
  age int,
...
> alter table test1
 ...
 /* primary key는 사용하는 것이고
  unique key는 같은 값이 등록되지 못하게 하는 용도이다 */
...
/*primary 방법 3개 있음! primary 단독으로 넣을 수도 있음*/

전체텍스트 index 
test1_name_idx -> 생략해도 된다
